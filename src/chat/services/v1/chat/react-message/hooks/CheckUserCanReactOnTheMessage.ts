...
import { MessageReaction_POST } from '../../../../../db_services/v1/message-reactions/interfaces/MessageReactionInterface';

const CheckUserCanReactOnTheMessage = () => async (context: HookContext) => {
    const { app, data } = context;
    const messageReactionData = data as MessageReaction_POST;
    const { message, reactor } = messageReactionData;
    ...
    if (!messageRecipientData) {
        throw new BadRequest('You can not react on this message.');
    } else {
        ...
        if (type === UserGroupType.PERSONAL_CHAT && firstUser && secondUser) {
            if (reactor.toString() !== firstUser.toString() && reactor.toString() !== secondUser.toString()) {
                throw new BadRequest('You can not react on the message for this chat.');
            }
        } else {
            ...
            if (!userGroupParticipantDataExists) {
                throw new BadRequest('You are not permitted to react on this message.');
            }
        }

        context.data = messageReactionData;
    }

    return context;
};

export default CheckUserCanReactOnTheMessage;
