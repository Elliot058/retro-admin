...
import getUserParticipantData from '../utils/getUserParticipantData';
...
const GetMessagesOfTheGroup = () => async (context: HookContext) => {
    ...
    const participantData = await getUserParticipantData(userData, query.group, app);
    // const pinnedMessages = await getPinnedMessages(app, query.group, userData);

    context.result = {
        messages: await messageRecipientModel.aggregate(aggregateQuery, { allowDiskUse: true }),
        participantData,
        // pinnedMessages,
    };
};

export default GetMessagesOfTheGroup;
